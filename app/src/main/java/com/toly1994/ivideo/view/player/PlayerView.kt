...
import com.toly1994.ivideo.presenter.player.IPlayerPresenter
import com.toly1994.ivideo.presenter.player.PlayerPresenter
 ...
 * 说明：视频播放视图层
 */
class PlayerView : AppCompatActivity(), ICtrlPanelView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val position = intent.getIntExtra("video-position", -1)
        val url = intent.getStringExtra("video-url")

        mPresenter = PlayerPresenter(this, id_vv)
        if (position != -1) {
            mPresenter.initVideo(position)
        }

        if (url != null && !"".equals(url)) {
            mPresenter.initVideo(url)
        }
    ...
    private fun doActions() {
        ...
        //进度条拖动监听
        id_sb_progress.setOnSeekBarChangeListener(
    ...
    override fun render(ctrlPanel: CtrlPanel) {
        Log.e("time", ": " + ctrlPanel.playedTime);
        ...
        id_sb_progress.secondaryProgress = ctrlPanel.secondRate
    }
